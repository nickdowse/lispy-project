...
void lval_print(lval* v){
	switch(v->type){
		...
		case LVAL_ERR:   printf("Error: %s", v->sym); break;
...
char* ltype_name(int t) {
	switch(t) {
		...
		case LVAL_QEXPR: return "S-Expression";
		case LVAL_SEXPR: return "Q-Expression";
...
// 	LASSERT(a, (syms->count == a->count-1), 
// 		"Function 'def' passed too many arguments for symbol. Got %i, expected %i",
...
lval* lval_call(lenv* e, lval* f, lval* a){
	...
	while (a->count) { 
		if (f->formals->count == 0){
			lval_del(a); return lval_err("Function passed too many arguments. Got %i, expected %i.",
				given, total);
		}
